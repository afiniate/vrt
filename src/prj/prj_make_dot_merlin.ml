...
let do_dot_merlin active_libs source_dirs build_dirs =
...
let monitor_dot_merlin active_libs source_dirs build_dirs () =
  Common.Cmd.result_guard
    (fun _ -> do_dot_merlin active_libs source_dirs build_dirs)

let spec =
  let open Command.Spec in
  empty
  +> flag ~aliases:["-l"] "--lib" (listed string)
    ~doc:"A library names to include"
  +> flag ~aliases:["-s"] "--source-dir" (listed string)
    ~doc:"A source directory to include"
  +> flag ~aliases:["-b"] "--build-dir" (listed string)
    ~doc:"A build directory to include"
